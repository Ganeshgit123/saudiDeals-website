...
export class ImagesLocationComponent {
      ...
      this.authService.getPostedPrpertyPost(this.propPostId).subscribe(
        (res: any) => {
          ...
          if (this.getPostedData.image == null) {
            ...
            this.previews = images.sort(function (first: any, second: any) { return (first.order || 0) - (second.order || 0); });
            this.locationImageForm = this.fb.group({
  ...
  changeProductImages(event: any) {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      if (currentImageCount + numberOfNewFiles <= 12) {
        ...
        this.imageLimt = true;
        this.toastr.error('Error', 'Max 12 images is allowed');
      }
    }
  }
  ...
  async applyWatermark(uploadedImage: HTMLImageElement, watermarkSrc: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      watermarkImage.onload = () => {
        ...
        const scale = 0.8; // scale factor of the watermark relative to base image
  ...
  postproduct() {
    ...
    if ((this.latitude == 0 && this.longitude == 0) && (!(this.currentLat && this.currentLng))) {
      this.locationNeed = true;
      return;
    } else {
      this.locationNeed = false;
    }
  ...
  onSubmitLocationImage(value) {
    ...
    // console.log("submit", this.locationImageForm.value);
    this.authService.propertyPostLevelUpdate(this.locationImageForm.value, postId)
      .subscribe((res: any) => {
        if (res.success == true) {
          this.toastr.success('Success', res.massage);
          this.router.navigate([`/property-post-ad/${this.category}/${this.subCateg}/extras/${this.propPostId}`]);
        } else {
  ...
  prevPage() {
    if (this.categIDd == 17) {
      ...
      this.router.navigate([`/property-post-ad/${this.category}/${this.subCateg}/specifications/${this.propPostId}`]);
    }
  }
  ...
  editProductImage(data) {
    ...
    this.authService.propertyPostLevelUpdate(data, this.propPostId)
      .subscribe((res: any) => {
        if (res.success == true) {
          this.toastr.success('Success', res.massage);
          this.router.navigate([`/property-post-ad/${this.category}/${this.subCateg}/extras/${this.propPostId}`]);
